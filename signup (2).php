...
$pass="";
		...
		if((!empty($UserN)) AND (!empty($pass)) ){
			...
			if($conn->query($sql)) {
		echo "signed up";//$copy="INSERT INTO login(username,password) VALUES('$Username','$Password')";
		//$paste=mysqli_query($conn,($copy));
		//$conn->query($paste);}
		//header("location: afterLogin.php",  true,  301 );  exit;  ;}}
		//}
